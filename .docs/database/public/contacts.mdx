...
| status             | text          | ステータス (`new`,`in_progress`,`closed`)                  | NOT NULL, DEFAULT 'new', CHECK (status IN ('new','in_progress','closed')) |
| priority           | text          | 優先度 (`low`,`normal`,`high`)                             | NOT NULL, DEFAULT 'normal', CHECK (priority IN ('low','normal','high')) |
