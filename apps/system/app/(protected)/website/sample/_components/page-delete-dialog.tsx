...
import { deleteSamplePage } from '~/_actions/sample';
...
export function PageDeleteDialog({ pageId, children }: PageDeleteDialogProps) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteSamplePage(pageId);
      if (result.success) {
        toast.success('サンプルページを削除しました');
        router.refresh();
      } else {
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>サンプルページを削除しますか？</AlertDialogTitle>
          <AlertDialogDescription>
            この操作は取り消せません。
          </AlertDialogDescription>
        </AlertDialogHeader>
