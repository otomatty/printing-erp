			...
			{navItems.map((item) => (
				<div key={item.label} className="relative">
					...
					{item.children && openDropdown === item.label && (
						<div
							ref={dropdownRef}
							className="absolute left-1/2 transform -translate-x-1/2 mt-3 w-max max-w-[90vw]  bg-white rounded-lg shadow-lg p-6 z-20 ring-1 ring-gray-200 ring-opacity-5"
						>
								...
								{item.children.map((child) => {
									...
									return (
										<div key={child.href} className="group w-40">
											...
											>
												<div className="flex items-center justify-center w-10 h-10 mb-2 rounded-md bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
